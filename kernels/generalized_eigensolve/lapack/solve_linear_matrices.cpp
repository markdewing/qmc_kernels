...
// Expanded dgeev (in preparation for scalapack version)
#include <string>
#include <hdf5.h>
#include <iostream>
#include <vector>
...
void compute_eigenthings_other(int N, double* overlap, double* hamiltonian, double *lowest_ev, double* scaled_eval)
{
  ...
  std::cout << "  Starting inverse" << std::endl;
  ...
  std::cout << "  Starting matrix multiply" << std::endl;
  double one(1.0);
  double zero(0.0);
  ...
  std::cout << "  Starting eigensolver" << std::endl;
  auto dgeev_start = std::chrono::system_clock::now();

  auto dgehrd_start = std::chrono::system_clock::now();
  int ilo = 1;
  int ihi = N;
  dgehrd(&N, &ilo, &ihi, prod.data(), &N, tau.data(), work.data(), &lwork, &info);
  if (info != 0) {
    std::cout << "dgehrd workspace query error, info = " << info << std::endl;
  }

  lwork = work[0];
  std::cout << "   optimal dgehrd lwork = " << lwork << std::endl;
  work.resize(lwork);

  dgehrd(&N, &ilo, &ihi, prod.data(), &N, tau.data(), work.data(), &lwork, &info);
  if (info != 0) {
    std::cout << "dgehrd error, info = " << info << std::endl;
  }
  auto dgehrd_end = std::chrono::system_clock::now();
  std::chrono::duration<double> dgehrd_time = dgehrd_end - dgehrd_start;
  std::cout << "   dgehrd time : " << dgehrd_time.count() << std::endl;
  ...
  dhseqr(&job, &compz, &N, &ilo, &ihi, prod.data(), &N, alphar.data(), alphai.data(), vr.data(), &N, work.data(), &lwork1, &info);
  if (info != 0) {
    std::cout << "dhseqr workspace query error, info = " << info << std::endl;
  }
  lwork1 = work[0];
  std::cout << "   optimal dhseqr lwork = " << lwork1 << std::endl;

  if (lwork1 > lwork) work.resize(lwork1);

  dhseqr(&job, &compz, &N, &ilo, &ihi, prod.data(), &N, alphar.data(), alphai.data(), vr.data(), &N, work.data(), &lwork1, &info);
  if (info != 0) {
    std::cout << "dhseqr error, info = " << info << std::endl;
  }
  ...
  auto dtrevc3_start = std::chrono::system_clock::now();
  char side('R');
  char howmny('B');
  std::vector<int> select(1);
  int nout;
  int lwork3 = -1;
  dtrevc3(&side, &howmny, select.data(), &N, prod.data(), &N, vl.data(), &N, vr.data(), &N, &N, &nout, work.data(), &lwork3, &info);
  lwork3 = work[0];
  std::cout << "   optimal dtrevc3 lwork = " << lwork3 << std::endl;
  if (lwork3 > lwork) {
    work.resize(lwork3);
  }
  ...
  dtrevc3(&side, &howmny, select.data(), &N, prod.data(), &N, vl.data(), &N, vr.data(), &N, &N, &nout, work.data(), &lwork3, &info);
  if (info != 0) {
    std::cout << "dtrevc3 error, info = " << info << std::endl;
  }
  auto dtrevc3_end = std::chrono::system_clock::now();
  std::chrono::duration<double> dtrevc3_time = dtrevc3_end - dtrevc3_start;
  std::cout << "   dtrevc3 time : " << dtrevc3_time.count() << std::endl;
  ...
  for (int i = 0; i < N; i++) {
    double eig = alphar[i];
    std::cout << std::setprecision(10) << i << " " << eig <<  " " << vr[i] << std::endl;
  }
#endif
